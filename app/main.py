...
- an HTTP middleware that logs every request to `logs/requests.log`,
...
app = FastAPI(title="Phi-2 Serving API", version="0.1.0", lifespan=lifespan)
...
async def log_requests(request: Request, call_next):
    ...
    For POSTs to `/generate*`, the request body is read to extract a
    truncated prompt preview, then rewound so handlers still see it.
    """
